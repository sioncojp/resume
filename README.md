...
# やりたいこと
自身のスキルの強みを伸ばすなら、EMやプレイングマネージャー、もしくは事業戦略の組み立てが得意なので推進する立ち位置にがベストです。

顧客やメンバーにワクワクするものを思考し実行するための推進する力が強いのが、私の強みだと思っております。

チャレンジをするなら、2点。

1つ目はインフラ ~ バックエンド ~ フロントなど色々触れるものの、もっとエンジニアのスペシャリストになっていきたいです。

例えば、フロントの表示速度やAPIのレイテンシーを高速化して保守性やパフォーマンス向上に貢献したり、よりコアな部分でユーザ価値に貢献したいです。

0 -> 1 フェーズが多く、またジェネラリスト的な動きで拾いに行くことが多かったので、プログラムでとことん突き詰めることは未経験なのでチャレンジしてみたいです。

2つ目はAI, ML周りはまだやったことないので、機会があればチャレンジしてみたいです。

# 自身の強み
自身の強みは、事業に責任を持ちリードすることです。

この強みを活かした業務を行いたいです。

顧客課題の本質を理解し中長期戦略と短期戦略に落とし込み、チームメンバーとグロースするためにをロードマップ、マイルストーンを作り上げ推進する力が強みです。

技術バックグラウンドもあるので、時にはコードを書いたり、エンジニア目線で戦略をセットすることも得意なので、現場ドリブンな組織であるほうが活躍すると思います。

実績として、1つ事業を構想し、立ち上げ。コードを書き、短期~中長期なにをしていくのか。

チームをどう構成し、メッセージをシンプルに発信していくか。

様々な部署を立ち上げ、ステークホルダーと会話しながら勧めてきました。

重圧、期待値、経験など高負荷の中で苦しみながら実績積んできたのが強みだと思います。

エンジニアからEMになるような役割の変え方ではなく、 コーポレートエンジニアしたり、CTO, COOなど大きくロールを変え続けられてきたのは他人にない強みです。

いろんなボール拾っていけるし、技術も責任持てる点が魅力的だと思っております。
...
- 新しい社内管理画面の構想と提案、現場の方と連携しながらGoとNext.js（GraphQL）で新規開発
  ...
  - 企画・提案、コーディングのリード（社内でGoとNext.jsの知見がないため）を実施
  - 技術スタック: Go
  	- GraphQL: gqlgen
	  - ORM: Gorm
